  ...
  if (fs.existsSync(ENV_FILE_PATH)) {
    ...
    log(
      `Warning: ${path.basename(
        ENV_FILE_PATH
      )} file not found. Relying on system environment variables.`
    );
  }
} catch (err) {
  error(
    `Failed to read or parse ${path.basename(ENV_FILE_PATH)}: ${err.message}`
  );
  process.exit(1);
}
...
const requiredValue =
  envConfig[REQUIRED_ENV_VAR] || process.env[REQUIRED_ENV_VAR];

if (!requiredValue) {
  error(
    `Required environment variable '${REQUIRED_ENV_VAR}' is not set or is empty.`
  );
  console.error(
...
try {
  if (!fs.existsSync(NODE_MODULES_DIR_PATH)) {
    ...
    if (installProcess.error) {
      error(
        `Failed to start dependency installation process: ${installProcess.error.message}`
      );
      process.exit(1);
    }
    if (installProcess.status !== 0) {
      error(
        `Dependency installation failed with exit code ${installProcess.status}.`
      );
      process.exit(1);
    }
...
log(
  `Launching development server using: ${DEV_SERVER_COMMAND} ${DEV_SERVER_ARGS.join(
    " "
  )}...`
);
try {
