...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    const showEggImage = !!(
        server.egg &&
        eggImageData &&
        eggImageData.image_enabled &&
        eggImageData.image_value
    );
    ...
    return (
        <ServerRowContainer className={className}>
            ...
            {showEggImage && eggImageData && (
                <div css={tw`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-[50px] w-auto z-10 pointer-events-none flex items-center justify-center`}>
                    <img
            ...
            {/* Resources Section */}
            <div css={tw`hidden sm:flex flex-col items-center justify-center col-span-12 sm:col-span-6 md:col-span-2 md:col-start-4 py-2`}>
                {!stats || isSuspended ? (
                    isSuspended ? (
