...
export default () => {
    ...
    const handleSubmission = ({ email }: Values, { setSubmitting, resetForm }: FormikHelpers<Values>) => {
        ...
        console.log(
            'DEBUG: CAPTCHA check passed. Calling requestPasswordResetEmail with email:',
            email,
            'and token:',
            token
        );
    ...
    return (
        <RequestPasswordPageWrapper>
            <StyledFormBox>
                <FormBackgroundLogo
                    src='/phoenixassets/phoenixpanel-transparent.png'
                    alt='Phoenix Panel background logo'
                />
                            ...
                            <>
                                <Form
                                    ...
                                    onFinish={() => {
                                        ...
                                        submitForm().catch((err: any) =>
                                            console.error('Error from submitForm promise:', err)
                                        );
                                    }}
                                >
                                    <Form.Item
                                        ...
                                        help={
                                            ...
                                            >
                                                Enter your account email address to receive instructions on resetting
                                                your password.
                                            </span>
                                        }
                                    >
                                ...
                                {console.log(
                                    'DEBUG: Inside render prop. showCaptchaOverlay:',
                                    showCaptchaOverlay,
                                    'enabled:',
                                    enabled,
                                    'provider:',
                                ...
                                {showCaptchaOverlay &&
                                    ...
                                    (provider === 'google' || provider === 'cloudflare') && (
                                        <div
                                            css={tw`fixed inset-0 bg-[#0F0F0F] bg-opacity-80 flex items-center justify-center transition-opacity duration-300 ease-out`}
                                            className={showCaptchaOverlay ? 'opacity-100' : 'opacity-0'}
                                        >
                                            {provider === 'google' && (
                                                <Reaptcha
                                                    ref={ref}
                                                    ...
                                                    onError={() => {
                                                        ...
                                                        addFlash({
                                                            type: 'error',
                                                            title: 'Error',
                                                            message: httpErrorToHuman('reCAPTCHA execution failed.'),
                                                        });
                                                    }}
                                                    onRender={() => {
                                                        console.log(
                                                            'DEBUG: Reaptcha onRender called. Component has rendered.'
                                                        );
                                                        // Execute reCAPTCHA only for Google provider after it has rendered
                                                        if (provider === 'google' && ref.current) {
                                                            console.log(
                                                                'DEBUG: Inside onRender. Attempting to execute Google reCAPTCHA.'
                                                            );
                                                            console.log('DEBUG: Site key being used:', siteKey);
                                                            console.log(
                                                                'DEBUG: Reaptcha ref.current state before execute:',
                                                                ref.current.state
                                                            );
                                                            ref.current.execute().catch((error: any) => {
                                                                console.error(
                                                                    'DEBUG: reCAPTCHA execution error from onRender:',
                                                                    error
                                                                );
                                                                setShowCaptchaOverlay(false);
                                                                setSubmitting(false);
                                                    ...
                                                    onLoad={() => {
                                                        console.log(
                                                            'DEBUG: Reaptcha onLoad called. Script has loaded.'
                                                        );
