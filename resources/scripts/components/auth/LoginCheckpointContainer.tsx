...
const EnhancedForm = withFormik<Props, Values>({
    handleSubmit: (
        { code, recoveryCode }: Values,
        {
            ...
            setStatus,
            setFieldValue,
            props: { clearAndAddHttpError, location },
        }: FormikHelpers<Values> & { props: Props }
    ) => {
        loginCheckpoint(location.state?.token || '', code, recoveryCode)
            ...
            .catch((error) => {
                ...
                if (
                    ...
                    error.response.data.errors[0].code === 'DisplayException' &&
                    error.response.data.errors[0].detail === 'The two-factor authentication token was invalid.'
                ) {
