    ...
    register({
      ...
      .catch((error: any) => {
        ...
        if (error.response && error.response.status === 409) {
          ...
          setErrors({
            email: 'A user with this username or email already exists.',
          });
        } else {
          clearAndAddHttpError({ error });
        }
          ...
          {({
            ...
            <Form onFinish={handleSubmit} layout='vertical' css={tw`relative z-10`}>
              ...
              <Form.Item
                ...
                help={
                  touched.email && errors.email ? <span css={tw`text-red-400 text-xs mt-1`}>{errors.email}</span> : null
                }
              >
                <StyledInput
                  name='email'
              ...
              <Form.Item
                ...
                validateStatus={touched.password_confirmation && errors.password_confirmation ? 'error' : undefined}
                help={
                  touched.password_confirmation && errors.password_confirmation ? (
                    <span css={tw`text-red-400 text-xs mt-1`}>{errors.password_confirmation}</span>
                  ) : null
                }
              >
                <StyledInputPassword
              ...
              {provider === 'cloudflare' && enabled === true ? (
                <div css={tw`mx-auto`}>
                  <div className='cf-turnstile' data-sitekey={siteKey} data-callback='javascriptCallback'></div>
