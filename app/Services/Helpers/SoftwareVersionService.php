...
use PheonixPanel\Exceptions\Service\Helper\CdnVersionFetchingException;

class SoftwareVersionService
{
    public const VERSION_CACHE_KEY = 'pheonixpanel:versioning_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://pheonixpanel.io/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('pheonixpanel.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('pheonixpanel.cdn.url'));
