...
(function (factory) {
    ...
    var AnsiUp = (function () {
        function AnsiUp() {
            ...
            this.htmlFormatter = {
                transform: function (fragment, instance) {
                    ...
                    if (instance._escape_for_html) txt = instance.old_escape_for_html(txt);
                    if (!fragment.bright && fragment.fg === null && fragment.bg === null) return txt;
                    var styles = [];
                    var classes = [];
                    var fg = fragment.fg;
                    var bg = fragment.bg;
                    if (fg === null && fragment.bright) fg = instance.ansi_colors[1][7];
        ...
        AnsiUp.prototype.detect_incomplete_link = function (txt) {
            ...
            if (!found) {
                if (/(https?:\/\/[^\s]+)/.test(txt)) return 0;
                else return -1;
            }
            var prefix = txt.substr(i + 1, 4);
            if (prefix.length === 0) return -1;
            if ('http'.indexOf(prefix) === 0) return i + 1;
        };
        AnsiUp.prototype.ansi_to = function (txt, formatter) {
            ...
            if (first_chunk.text.length > 0) blocks.unshift(formatter.transform(first_chunk, this));
        ...
        AnsiUp.prototype.handle_incomplete_sequences = function (chunks) {
            ...
            if (last_chunk.length > 0 && this.detect_incomplete_ansi(last_chunk)) {
                ...
                if (chunks.length === 2 && chunks[1] === '' && chunks[0].slice(-1) === '\x1B') {
                    this._buffer = '\x1B';
